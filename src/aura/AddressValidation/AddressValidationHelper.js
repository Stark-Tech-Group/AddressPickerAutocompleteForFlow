...
({
  ...
  setValidation: function (cmp) {
    let labels = {
      SEARCH_AND_ALL_ADDRESS_FIELDS_REQUIRED: $A.get(
        "$Label.c.Search_and_All_Address_Fields_Required"
      ),
      SEARCH_AND_ADDRESS_FIELD_REQUIRED: $A.get(
        "$Label.c.Search_and_Address_Field_Required"
      ),
      FILLED_ADDRESS_FIELDS_REQUIRED: $A.get(
        "$Label.c.Filled_Address_Fields_Required"
      ),
    };
    ...
    cmp.set("v.validate", function () {
      ...
      showCountyField
        ? addressFields.push(cmp.get("v.administrative_area_level_2"))
        : "";
      ...
      if (fieldsRequired && isRequired) {
        if (locationValue.length > 0 && !addressFieldEmpty) {
          ...
          return {
            isValid: false,
            errorMessage: labels.SEARCH_AND_ALL_ADDRESS_FIELDS_REQUIRED,
          };
        }
      } else if (!fieldsRequired && isRequired) {
        if (locationValue.length > 0) return { isValid: true };
        else
          return {
            isValid: false,
            errorMessage: labels.SEARCH_AND_ADDRESS_FIELD_REQUIRED,
          };
      } else if (showAddressFields && fieldsRequired) {
        if (!addressFieldEmpty) return { isValid: true };
        else
          return {
            isValid: false,
            errorMessage: labels.FILLED_ADDRESS_FIELDS_REQUIRED,
          };
      }
    });
  },
  initialiseMapData: function (cmp, helper) {
    let lat = cmp.get("v.latitude"),
      lng = cmp.get("v.longitude");
    let labels = {
  ...
  startSearch: function (cmp) {
    ...
    searchTimeout = window.setTimeout(
        ...
        this.callServer(
          ...
          function (response) {
            ...
            if (resp.status === "OK") {
              ...
              cmp.set(
                "v.apiError",
                resp.error_message + " " + labels.CONTACT_ADMIN
              );
            } else if (resp.status == "ZERO_RESULTS") {
              // If no results found
              cmp.set("v.predictions", null);
            } else {
              // If an unknown error is returned
      ...
      300
    ); // Wait for 300 ms before sending search request
    ...
    this.callServer(
      ...
      function (response) {
        ...
        if (placeDetails.status === "OK") {
          ...
          this.validateAddress(cmp);
        } else if (placeDetails.error_message) {
          // If an error message is returned
          cmp.set(
            "v.apiError",
            placeDetails.error_message + " " + labels.CONTACT_ADMIN
          );
        } else {
          cmp.set("v.apiError", labels.API_UNKNOWN_ERROR);
        }
  ...
  validateAddress: function (cmp) {
    let addressInput = cmp.find("fullAddress");
